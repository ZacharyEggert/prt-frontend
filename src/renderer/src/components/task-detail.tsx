...
import { Edit, Check, TestTube, Trash2, AlertCircle, ArrowRight, Lock } from 'lucide-react'
import {
  getStatusBadgeProps,
  getTypeBadgeProps,
...
function DependencyItem({
  ...
  if (!task) {
    return (
      <div className="flex items-center gap-2">
        <ArrowRight className="size-3 text-muted-foreground" />
        <span className="text-sm text-muted-foreground">{taskId} (not found)</span>
      </div>
    )
  }
...
export function TaskDetail({
  ...
  // Dependency and blocks data
  const dependencyIds = task?.['depends-on'] || []
          ...
          <div className="space-y-4 pb-4">
            ...
            {dependencyIds.length > 0 && (
              <>
                <Separator />
                <div className="space-y-2">
                  <h3 className="text-sm font-semibold">Dependencies ({dependencyIds.length})</h3>
            ...
            {blocksIds.length > 0 && (
              <>
                <Separator />
                <div className="space-y-2">
                  <h3 className="text-sm font-semibold">Blocks ({blocksIds.length})</h3>
                  <div className="space-y-1">
                    {blocksIds.map((blockId) => (
                      <DependencyItem key={blockId} taskId={blockId} clickable={false} />
                    ))}
                  </div>
                </div>
              </>
            ...
            <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    This will permanently delete &quot;{task.title}&quot; ({task.id}) and remove all
                    references from other tasks. This action cannot be undone.
                  </AlertDialogDescription>
                </AlertDialogHeader>
